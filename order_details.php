...
if (strlen($act) == 0 || $act == "del" || $act == "edit") {
  ...
  while ($o = mysql_fetch_object($r)) {
    ...
    echo "<tr><form action='' method=post>
      ...
      <td><input name=ns size=40 value='". $o->ProductID . "' $chg'></td>
      <td><input name=salary size=20 value='". $o->UnitPrice . "' $chg;text-align:right'></td>
...
# ส่วนเพิ่มข้อมูล
if ($act == "add") {
  $q  = "insert into orderdetails (OrderID,ProductID,UnitPrice)values('". $_REQUEST{'id'} . "','". $_REQUEST{'ns'} . "','". $_REQUEST{'salary'} . "')";
  $conn = mysql_connect("$host","$user","$password");
...
# ส่วนลบข้อมูล
if ($act == "del : confirm") {
  $q  = "delete from orderdetails where OrderID ='". $_REQUEST{'id'} . "'";
  $conn = mysql_connect("$host","$user","$password");
  $r = mysql_db_query($db,$q);   
...
# ส่วนแก้ไขข้อมูล
if ($act == "edit : confirm") {
  $q  = "update $tb set ProductID ='". $_REQUEST{'ns'} . "', UnitPrice ='". $_REQUEST{'salary'} . "' where OrderID=" . $_REQUEST{'id'};
  $conn = mysql_connect("$host","$user","$password");
  $r = mysql_db_query($db,$q);   
