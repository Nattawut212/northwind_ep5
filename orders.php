...
if (strlen($act) == 0 || $act == "del" || $act == "edit") {
  ...
  while ($o = mysql_fetch_object($r)) {
    ...
    echo "<tr><form action='' method=post>
      ...
      <td><input name=ns size=40 value='". $o->CustomerID . "' $chg'></td>
      <td><input name=salary size=20 value='". $o->EmployeeID . "' $chg;text-align:right'></td>
...
# ส่วนเพิ่มข้อมูล
if ($act == "add") {
  $q  = "insert into orders (OrderID,CustomerID,EmployeeID)values('". $_REQUEST{'id'} . "','". $_REQUEST{'ns'} . "','". $_REQUEST{'salary'} . "')";
  $conn = mysql_connect("$host","$user","$password");
...
# ส่วนลบข้อมูล
if ($act == "del : confirm") {
  $q  = "delete from orders where OrderID ='". $_REQUEST{'id'} . "'";
  $conn = mysql_connect("$host","$user","$password");
  $r = mysql_db_query($db,$q);   
...
# ส่วนแก้ไขข้อมูล
if ($act == "edit : confirm") {
  $q  = "update $tb set CustomerID ='". $_REQUEST{'ns'} . "', EmployeeID ='". $_REQUEST{'salary'} . "' where OrderID=" . $_REQUEST{'id'};
  $conn = mysql_connect("$host","$user","$password");
  $r = mysql_db_query($db,$q);   
