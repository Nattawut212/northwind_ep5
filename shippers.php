...
if (strlen($act) == 0 || $act == "del" || $act == "edit") {
  ...
  while ($o = mysql_fetch_object($r)) {
    if (isset($_REQUEST{'id'}) && $_REQUEST{'id'}  == $o->ShipperID) $chg = " style='background-color:#f9f9f9"; else $chg = " readonly style='background-color:#ffffdd";
    echo "<tr><form action='' method=post>
      <td><input name=id size=5 value='". $o->ShipperID . "' style='background-color:#dddddd' readonly></td>
      <td><input name=ns size=40 value='". $o->CompanyName  . "' $chg'></td>
      <td><input name=salary size=20 value='". $o->Phone . "' $chg;text-align:right'></td>
...
# ส่วนเพิ่มข้อมูล
if ($act == "add") {
  $q  = "insert into shippers (ShipperID,Phone,CompanyName)values('". $_REQUEST{'id'} . "','". $_REQUEST{'ns'} . "','". $_REQUEST{'salary'} . "')";
  $conn = mysql_connect("$host","$user","$password");
...
# ส่วนลบข้อมูล
if ($act == "del : confirm") {
  $q  = "delete from shippers where ShipperID  ='". $_REQUEST{'id'} . "'";
  $conn = mysql_connect("$host","$user","$password");
  $r = mysql_db_query($db,$q);   
...
# ส่วนแก้ไขข้อมูล
if ($act == "edit : confirm") {
  $q  = "update $tb set CompanyName ='". $_REQUEST{'ns'} . "' Phone =" . $_REQUEST{'salary'} . "' where ShipperID =" . $_REQUEST{'id'};
  $conn = mysql_connect("$host","$user","$password");
  $r = mysql_db_query($db,$q);   
